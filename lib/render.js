...
import f from '@turf/helpers';
const featureCollection = f.featureCollection;
...
export function render(container, id, options) {
    ...
    getChangeset(changesetId, options.overpassBase, function(err, result) {
        ...
        document.querySelector(
            '.cmap-c-link-osmhv'
        ).href = 'http://osmhv.openstreetmap.de/changeset.jsp?id=' +
            changesetId;
        document.querySelector(
            '.cmap-c-link-josm'
        ).href = 'http://127.0.0.1:8111/import?url=http://www.openstreetmap.org/api/0.6/changeset/' +
            changesetId +
            '/download';
        document.querySelector(
            '.cmap-c-link-id'
        ).href = 'http://preview.ideditor.com/release#map=15/' +
            center.lat +
            '/' +
            center.lng;
        ...
        document.querySelector(
            '.cmap-u-link-disc'
        ).href = 'http://resultmaps.neis-one.org/osm-discussion-comments?uid=' +
            userId;
        document.querySelector(
            '.cmap-u-link-comm'
        ).href = 'http://resultmaps.neis-one.org/osm-discussion-comments?uid=115894' +
            userId +
            '&commented';
        ...
        var layerSelector = document.querySelector(
            '.cmap-filter-action-section'
        );
        ...
        var baseLayerSelector = document.querySelector(
            '.cmap-map-style-section'
        );
...
function renderHTML(container) {
    ...
    var diffMetadata = elt('div', {
        class: 'cmap-diff-metadata cmap-scroll-styled',
        style: 'display: none'
    });
    var diffTags = elt('div', {
        class: 'cmap-diff-tags cmap-scroll-styled',
        style: 'display: none'
    });
    var diff = elt(
        'div',
        { class: 'cmap-diff', style: 'display: none' },
        diffMetadata,
        diffTags
    );
            ...
            elt(
                ...
                elt(
                    'a',
                    {
                        target: '_blank',
                        class: 'cmap-hlist-item cmap-pointer cmap-noselect',
                        href: '//osmlab.github.io/osm-deep-history/#/' +
                            type +
                            '/' +
                            id
                    },
        ...
        if (ignoreList.indexOf(prop) === -1) {
            ...
            types.forEach(function(type) {
                if (diff[prop].hasOwnProperty(type)) {
                    var propClass = 'diff-property cmap-scroll-styled props-diff-' +
                        type;
...
function filterLayers() {
    ...
    layers.forEach(function(layer) {
        var isSelectedAction = selectedActions.reduce(
            function(accum, action) {
                return layersKey[layer][action] || accum;
            },
            false
        );
        var isSelectedType = selectedTypes.reduce(
            function(accum, type) {
