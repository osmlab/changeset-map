...
export class Map {
      ...
      relationMembers.forEach(
        member => {
          member.geometry.coordinates = member.geometry.coordinates.filter(
            ...
            i => typeof(i) === 'number' || (typeof(i) === 'object' && i[0] !== 0)
          );
        }
      );
  ...
  addMapLayers() {
    ...
    this.map.addLayer({
      ...
      filter: ['all', ['==', 'type', 'node'], ['==', 'changeType', 'added']]
    });
  ...
  highlightRelationMemberFeature(featureId) {
    this.map.setFilter('highlight-relation-member-line', ['==', 'ref', featureId]);
    this.map.setFilter('highlight-relation-member-point', ['==', 'ref', featureId]);
  }
  clearHighlight() {
