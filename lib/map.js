...
export class Map {
    ...
    renderMap(baseLayer, result) {
        ...
        if (this.map) {
            console.log(this.map.loaded(), this.mapLoaded);
            if (!this.mapLoaded || !this.map.loaded()) {
            ...
            if (baseLayer && this.baseLayer !== baseLayer) {
                this.map.setStyle(baseLayer);
                this.baseLayer = baseLayer;
                setTimeout(() => this.renderMap(baseLayer, result), 900);
            } else {
