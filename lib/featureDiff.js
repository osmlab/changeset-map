...
export const displayDiff = function(
  ...
  if (featuresWithId[0].properties.type === 'relation') {
    ...
    reactDOM(<RelationMembersTable featuresWithId={featuresWithId} />, membersContainer);
  }
};
...
const RelationMembersTable = function({ featuresWithId }) {
  return (
    <table
      ...
      style={featuresWithId[0].properties.changeType === 'added' ? { width: '350px' } : undefined}
    >
      <thead>
        <tr>
          <td className="cmap-table-head" colSpan="2"><span className="cmap-strong">MEMBERS</span> (click to highlight)</td>
        </tr>
      </thead>
      <tbody>
        {featuresWithId[0].properties.relations.map((i, n) =>
          <tr key={n} onClick={() => cmap.emit('selectMember', i.properties.ref)}>
            <th className="cmap-strong cmap-pointer">
              {`${i.properties.ref} (${i.properties.type.toUpperCase()})`}
            </th>
            <td className="diff-property cmap-scroll-styled cmap-pointer">{i.properties.role}</td>
          </tr>
        )}
      </tbody>
    </table>
